...
//------------------------Parameter----------------------
`define TV_IN_gmem1 "../tv/cdatafile/c.top_wrapper.autotvin_gmem1.dat"
`define TV_OUT_gmem1 "../tv/rtldatafile/rtl.top_wrapper.autotvout_gmem1.dat"
...
parameter gmem1_transaction_depth = 32'd 1210;
parameter gmem1_mem_depth = 32'd 1210;
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > gmem1_mem_depth) begin
                $display ("/home/lorenzor/workspace/convolution/convolution/solution_0/sim/verilog/AESL_axi_master_gmem1.v: Write request address %d exceed AXI master gmem1 array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, gmem1_mem_depth); 
                $finish;
            end
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > gmem1_mem_depth) begin
                $display ("/home/lorenzor/workspace/convolution/convolution/solution_0/sim/verilog/AESL_axi_master_gmem1.v: Read request address %d exceed AXI master gmem1 array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, gmem1_mem_depth); 
                $finish;
            end
...
initial begin : runtime_depth_init_process
    gmem1_port0_fresh_depth[0] = 1210;
...
initial begin : write_file_proc 
  ...
  while(1) begin 
      ...
      bin_data = gmem1_cur_trans_depth[transaction_num];
      ...
      for(i = 0; i < gmem1_mem_depth; i = i + 1) begin 
          ...
          for (j=0;j<1;j=j+1) begin
              ...
              endcase
              if (port_idx == 0) begin
                  if (port_write_done[port_idx] == 1'b0 && write_idx == gmem1_port0_fresh_depth[transaction_num]) begin
                  ...
                  if (port_fill_done[port_idx] == 1'b0 && port_write_done[port_idx] == 1'b1 && write_idx == gmem1_port0_blank_depth[transaction_num]) begin
                      write_idx = 0;
                      port_fill_done[port_idx] = 1'b1;
                      port_idx = port_idx + 1;
                  end
