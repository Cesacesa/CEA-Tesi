...
typedef struct {
    ...
    XTop_wrapper_uio_map maps[ MAX_UIO_MAPS ];
} XTop_wrapper_uio_info;

/***************** Variable Definitions **************************************/
static XTop_wrapper_uio_info uio_info;
...
static int uio_info_read_name(XTop_wrapper_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XTop_wrapper_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XTop_wrapper_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XTop_wrapper_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XTop_wrapper_Initialize(XTop_wrapper *InstancePtr, const char* InstanceName) {
	XTop_wrapper_uio_info *InfoPtr = &uio_info;
    ...
    // NOTE: slave interface 'Gmem3' should be mapped to uioX/map2
    InstancePtr->Gmem3_BaseAddress = (u64)mmap(NULL, InfoPtr->maps[2].size, PROT_READ|PROT_WRITE, MAP_SHARED, InfoPtr->uio_fd, 2 * getpagesize());
    assert(InstancePtr->Gmem3_BaseAddress);
...
int XTop_wrapper_Release(XTop_wrapper *InstancePtr) {
    ...
    munmap((void*)InstancePtr->Gmem3_BaseAddress, InfoPtr->maps[2].size);
