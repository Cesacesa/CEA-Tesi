...
template<
    ...
    int FW_IN, int FH_IN,
    int OCH, int OW, int OH, // OCH, OW, OH non sono usati direttamente qui ma mantenuti per consistenza
    int ICH_PAR_IN,
    int WINDOW_IN,
    int STRIDE,
    int ICH_PAR_OUT>
    ...
    hls::stream<conv_packet_t<FW_IN, FH_IN, ICH_PAR_IN>> &conv_data_stream)
{
    ...
    int MEM_IN_T_BIT_WIDTH = 64; // bit width of each packet element
                    ...
                    for(int s_ich = 0; s_ich < ICH; s_ich += ICH_PAR_IN){
                        ...
                        for (int s_ich_par = 0; s_ich_par < ICH_PAR_IN; s_ich_par++){
                            ...
                            for (int i = 0; i < FW_IN * FH_IN * ICH_PAR_IN; i++) {
                                ap_uint<64> slice = current_packet.range((i+1)*64 - 1, i*64);
                                std::cout << "Packet slice[" << i << "]: 0x" << slice.to_string(16) << std::endl;
                            }
                            #endif
...
template< int ICH, int IW, int IH,
          int FW_IN, int FH_IN,
          int OCH, int OW, int OH,
          int ICH_PAR_IN, int OUT_SIZE_IN, int STRIDE,
                 ...
                 hls::stream<mem_out_t> &memory_out_stream)
{
    ...
    out_conv2mem<CONV_0_OCH, CONV_0_OW, CONV_0_OH, CONV_1_FW, CONV_1_FH, CONV_1_OCH, CONV_1_OW, CONV_1_OH, CONV_1_ICH_PAR, CONV_1_OUTPUT_SIZE, CONV_1_STRIDE, CONV_2_ICH_PAR, CONV_2_FW, CONV_2_FH, CONV_2_OCH, WINDOW_OUT_2>(out_mem2, memory_out_stream); // read partitioned memory and write to output stream
}
